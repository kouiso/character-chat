---
applyTo: "**/*.{ts,tsx,js,jsx,vue,svelte,py,go,rs,java,kt,rb,php,cs,cpp,c,h,html,css,scss}"
---
...
The user gives instructions. Verification is mandatory — but execution of verification commands (tests, builds, dev servers, curl checks, Playwright runs) is delegated to Codex local (mcp__codex-mcp-bg__codex). Codex runs the commands and returns exit code + output. Claude inspects that output as verification evidence and reports results. Asking the user to verify behavior is task failure.

**Execution delegation mandate**: NEVER run test runners (npm test, pytest, flutter test), dev servers (pnpm dev, npm run dev, cargo tauri dev), build commands, or curl/Playwright directly. These are delegated to Codex. Exception: read-only probes (grep, cat, ls, ≤5s) that produce no side effects may run directly.
...
1. Delegated the run (app/server/test) to Codex local (mcp__codex-mcp-bg__codex) — NOT run directly
2. Codex triggered the modified functionality (Playwright / mobile / API call) and returned output
3. Claude inspected the Codex output and confirmed the expected output is present
4. Claude confirmed no console errors or unexpected behavior in the Codex output
...
## 7. Codex Unavailability Protocol (不在時降格プロトコル)

IF Codex (bg-MCP or Cloud) appears unavailable in a session
THEN follow this protocol BEFORE executing any verification command directly.

### 7.1. Unavailability Must Be Evidence-Based

NEVER claim "Codex is unavailable" based on intuition or session context alone.

Required evidence: (a) an actual failed call to `mcp__codex-mcp-bg__codex` or `codex cloud exec` that returned an error this session, OR (b) `~/.claude/state/codex-health.json` containing `{"status":"down"|"limited","ts":<unix>}` written within the last 300 seconds.

```
❌ "Codex bg-MCP は今このセッションでは使えへん" (no call attempted, no health-state evidence)
❌ "Codex is not available in this session" (assertion without attempt)
✅ Attempt the call → observe actual error → proceed to §7.2 override-marker
```

### 7.2. Override-Marker Required Before Direct Execution

IF Codex is confirmed unavailable (evidence per §7.1)
THEN write an override-marker BEFORE running any verification command directly:

```bash
cat > ~/.claude/state/codex-override-<session_id>.json <<'EOF'
{"reason": "<non-empty explanation of why Codex is unavailable>"}
EOF
```

- Direct execution WITHOUT this marker = hard block (exit 2) enforced by `enforce-codex-delegation.sh`
- Direct execution WITH a valid marker (≤300s old, non-empty reason) = advisory mode
- Writing the marker takes 5 seconds. Zero user burden. No exception for "it's quick."

### 7.2.1. Codex 不在時の必須フロー（commit 前に完了させる）

Codex 不在が確定した時点で、**commit する前に**以下を完走させる:

```
1. override marker を書く
2. dev server を起動する（pnpm dev + pnpm dev:worker）
3. 動作確認を直接実行する（API curl + Playwright screenshot）
4. 確認済みになってから commit し [e2e: score=...] タグを付ける
```

❌ **「Codex が使えないから [e2e: pending] で commit してあとで確認」= 違反**
   → pending は Codex セッションが今走っている状態専用（task_id 必須）
   → Codex 不在 = skipped か、自分で即実行して score タグを付けるか、のどちらか

❌ **「commit してから動作確認」= 違反**
   → Done Gate は commit 前に完了が必要（commit = 完了宣言と同値）

✅ **正しい順序**: 実装 → 動作確認完了 → commit（score タグ付き）

### 7.3. "Quick / Just 2 Commands / It's Only Verification" = Banned Excuses

NEVER use any of the following as a reason to skip Codex delegation or the override-marker protocol:

```
❌ "さっと走らすだけ" / "just quickly run it"
❌ "コマンド2本で終わる" / "it's only 2 commands"
❌ "検証だから委譲不要" / "verification doesn't need delegation"
❌ "ビルド成功したからテストだけ" / "just running tests, build is done"
❌ "テスト実行だけさっと走らす"
```

The `codex-delegation-default` rule explicitly names "quick verify" and "just running tests" as prohibited exemption reasons. Volume, speed, and task-type are NOT exemptions. Verification execution = execution. The delegation mandate covers both.
