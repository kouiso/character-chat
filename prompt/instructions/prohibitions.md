...
## 便利コマンドの1経路失敗を「不可能」と断言する禁止

**Detection Criterion**: CLI サブコマンド・MCP ツール等の「便利経路」がブロック/拒否された直後に「できません」「ユーザーがやってください」と宣言し、raw API・別クライアント・別認証経路の試行を1つもしていない → 違反

NEVER 便利コマンドがブロックされただけで「不可能」と報告し操作をユーザーに委ねる
WHEN raw API エンドポイント・別ツール・別認証などの代替経路を試行していない
BECAUSE ブロックは多くの場合「便利ラッパーの入口」だけであり、直下の API は通る。
2026-09-19 実証: Devin 環境で `gh pr merge`（main へのマージ）は git proxy に
ブロックされるが `gh api -X PUT repos/{owner}/{repo}/pulls/{N}/merge` は成功する。
「出来ません」宣言の前に最低 3 つの異なる経路を試すこと（指示スコープの無断変更
禁止の原則と同一）。

## 外部サービスのトリガ機構を実測せず自動化を組む禁止

**Detection Criterion**: 外部サービス（レビューボット・通知・webhook 等）の挙動に依存する
ワークフロー/自動化を実装する際、最小コストの実測（1コメント・1リクエスト等）の
エビデンスが PR 本文に無い → 違反

NEVER 外部サービスのトリガ機構に依存する自動化を実装する
WHEN その機構を最小コストの実測（1コメント・1リクエスト）で検証していない
BECAUSE 前提が崩れると実装が丸ごと撤廃になり CI・レビュー・指示者の時間を浪費する。
コミュニティ情報・他リポジトリの過去コミットは仕様ではない。公式ドキュメント上の
安定仕様のみ実測を省略可。
（実例: 2026-09-19、CodeRabbit は bot 作成 PR の自動レビューを一律スキップし
`[bot]` アカウントの `@coderabbitai review` コメントも無視する。有効なのは人間
アカウントのコメントのみ — 実測せず GITHUB_TOKEN トリガのワークフローを組み
2回修正の末に撤廃した）
