#!/usr/bin/env bash
# offload-images-to-gdrive.sh — Phase 2: 画像実体を Drive へ退避 + ローカル削除
# 削除ゲート: 個別ファイル単位 rclone check (MD5) 一致のみ削除
# 順序: ①Drive退避 → ②ハッシュ検証 → ③ローカル削除
#
# 使用法: bash script/offload-images-to-gdrive.sh [--dry-run]
set -euo pipefail
...
offload_file() {
  local src_abs="$1" dst_prefix="$2" dst_name="$3"
  local dst_path="${dst_prefix%/}/${dst_name}"
  ((TOTAL++)) || true

  if $DRY_RUN; then log "DRY: $(basename "$src_abs") → $dst_path"; return; fi

  if ! rclone copyto "$src_abs" "$dst_path" 2>/tmp/rclone-err.txt; then
    err "copy failed: $src_abs → $(head -1 /tmp/rclone-err.txt)"; return
  fi
  ((UPLOADED++)) || true

  local tmpdir; tmpdir=$(mktemp -d)
  cp "$src_abs" "$tmpdir/${dst_name}"
  if rclone check "$tmpdir" "$dst_prefix" --include="${dst_name}" 2>/tmp/rclone-check-err.txt; then
    ((VERIFIED++)) || true
    rm -rf "$tmpdir"
    rm -f "$src_abs"
    ((DELETED++)) || true
    ...
    head -2 /tmp/rclone-check-err.txt >&2
    rm -rf "$tmpdir"
  fi
}
...
offload_avatar_dirs() {
  ...
  for dir in "${TARGET_DIRS[@]}"; do
    ...
    while IFS= read -r -d '' file; do
      ...
      offload_file "$file" "gdrive:avatars/${char}" "$dst_name"
...
offload_e2e_images() {
  ...
  for run_dir in "$runs_abs"/*/; do
    ...
    while IFS= read -r -d '' file; do
      is_image "$file" || continue
      offload_file "$file" "gdrive:evals/${year_month}/${run_id}" "$(basename "$file")"
    done < <(find "$run_dir" -type f -print0 2>/dev/null)
  done
}

log "=== Drive 退避開始 (dry_run=$DRY_RUN) ==="
offload_avatar_dirs
offload_e2e_images
log ""
log "=== 完了 ==="
