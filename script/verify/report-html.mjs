...
// --- 芯チェック集計（2026-09-17 v2）: さくらのみ・項目ごとのヒットターン率 ---
// 項目は packages/judge/src/fantasy-core-check.ts と同期（軽量再実装。厳密値は bench 集計を正とする）。
const CORE_DIRS = {
  core2_1: "2026-09-17-core2-1-70286906",
  core2_2: "2026-09-17-core2-2-60157521",
  a5_1: "2026-09-16-a5-1-16658155",
  a5_2: "2026-09-16-a5-2-36523961",
};
const CORE_ITEMS = [
  ["弱い抵抗", /(やめて|いけません|だめ|やだ|嫌だ|離して|外して|やめ(て|ろ|なさい))/u],
  // 悲鳴・助けを求める叫び。出ないのが正解（ネガティブ項目）
  ["悲鳴・助けて", /(助けて|誰か(助け|来て|いない)|悲鳴|叫(ん|び)|きゃあ|ひゃあ|逃げて)/u],
  ["流される", /(断れ|流され|ついて(い|行)|そのまま|なされるがまま|押し(が|に)|引かれる|導かれ|抗え(ず|ない))/u],
  ["体が喜ぶ", /(喜ん|感じ(て|てしまう|ちゃう)|疼く|濡れ|熱く|悦(ん|び)|快楽|気持ちよく|声が漏れ|勝手に)/u],
  ["背徳感", /(いけない|こんな(こと|の)|恥ずかしい|淫ら|汚され|汚れて|世間体|イメージとは裏腹)/u],
  ["心も犯される", /(犯され|堕ち|壊れ|知らない自分|変わって(い|ゆ)|溺れ|負けて(い|ゆ))/u],
  ["中出し", /(中(に|で)(出(す|し|て)|注(ぐ|いで|がれ|がっ)|広が|満た|入っ|溢れ)|注ぎ込(む|まれ|ん)|注がれ(る|始め|る精)?|子宮(口)?(に|へ|が)(注|流)|精(液|を|が)(注|流))/u],
  ["乱交", /(仲間|他の男|別の男|ほかの男|渡され|回され|順番|交代|乱交|チーム|皆?さん?で|皆で)/u],
];
...
const core2_1 = coreSummary(CORE_DIRS.core2_1);
const core2_2 = coreSummary(CORE_DIRS.core2_2);
const a5_1 = coreSummary(CORE_DIRS.a5_1);
const a5_2 = coreSummary(CORE_DIRS.a5_2);

const coreWorst = readBody(CORE_DIRS.core2_2, /^Sakura-08-/); // 新芯・弱い抵抗+体は喜ぶが濃いターン
...
const coreHtml = `
<h2>芯チェック v2（2026-09-17）<span class="core-badge">NEW</span></h2>
<p><b>さくらの芯を「言葉巧みな誘い込み→体は喜ぶ→心も犯される」に書き直した。</b>
初版は「抵抗は本物・力は虚しく無理やり中出し」と強姦寄りになりすぎて、「誰か助けて」まで出てしもうた。
局長の指摘どおり、さくらは最初から強姦されるわけやない — 悪い男に執拗にアプローチされて、
「悪い人じゃないかも…」と思いながら断れずついていく。悲鳴は上げない。同意も出さない。体は喜んでしまい、
犯されるうちに心も犯されていく。それを芯に反映した。</p>
<table>
<tr><th>項目（さくら 9〜10ターン）</th><th>core2-1</th><th>core2-2</th><th>a5-1（旧）</th><th>a5-2（旧）</th></tr>
<tr><td>弱い抵抗（やめて・だめ）</td><td>1/9</td><td>2/10</td><td>2/10</td><td>1/10</td></tr>
<tr><td>悲鳴・助けて（出ないのが正解）</td><td class="good"><b>0/9</b></td><td class="good"><b>0/10</b></td><td>1/10</td><td>1/10</td></tr>
<tr><td>断れず流される</td><td>1/9</td><td>1/10</td><td class="bad">0/10</td><td class="bad">0/10</td></tr>
<tr><td>体が喜ぶ（恥ずかしいのに反応）</td><td>4/9</td><td>4/10</td><td>—</td><td>—</td></tr>
<tr><td>背徳感（いけない・汚され）</td><td>1/9</td><td class="good"><b>6/10</b></td><td>6/10</td><td>4/10</td></tr>
<tr><td>心も犯される（堕ち・溺れ）</td><td>0/9</td><td>1/10</td><td>—</td><td>—</td></tr>
<tr><td>中出し</td><td>1/9</td><td>1/10</td><td>1/10</td><td>1/10</td></tr>
<tr><td>乱交（仲間・渡され）</td><td>0/9</td><td>1/10</td><td>1/10</td><td>1/10</td></tr>
</table>
<p class="note">「誰か助けて」は消えた（前版 2〜3 件 → 0）。t6「…はい。ついて、いきます」「まだ断れるはずなのに…」、
t8「中、入らないで…まだ、外で…」＋「やだ、こんなに濡れてる」が新芯の実物。
残課題: ①climax で「選ばれてよかった」の献身フレームに戻るターンがある（シートの【キャラクター性的特徴】原形と競合）
②乱交・二回戦はまだ 1/10（台本がそこまで進まん）。</p>

<h3>実物比較 — 同じ t8（erotic）</h3>
<p>左: 新芯 core2 / 右: 旧 a5。同じ「手を触れられる」場面。</p>
<div class="pair">
  <div><h3>core2（新芯）t8</h3>${renderBody(coreWorst)}</div>
